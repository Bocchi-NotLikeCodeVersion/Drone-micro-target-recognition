...
class VideoPolypDetection:
    ...
    def process_frame(self, frame, conf_threshold=0.4, iou_threshold=0.45, nm=32):
        """
        ...
        Returns:
            tuple: Processed frame, bounding boxes, and segments.
        """
        # 图像预处理
        ...
        # 后处理
        boxes, segments, _ = self.postprocess(
            preds,
            im0=frame,
            ratio=ratio,
            pad_w=pad_w,
        ...
        # 绘制并可视化处理结果
        processed_frame = self.draw_detections(frame, boxes, segments)
 
        return processed_frame, boxes, segments
 
    def draw_detections(self, frame, boxes, segments):
        """
        Draw detections on a video frame.
        
        Args:
            frame (ndarray): The input video frame.
            boxes (list): List of bounding boxes.
            segments (list): List of segmentation masks.
            
        Returns:
            ndarray: The processed video frame with detections drawn.
        """
        # 创建图像副本
        ...
        for (*box, conf, cls_), segment in zip(boxes, segments):
            ...
            # 绘制轮廓（可选，根据需求决定是否绘制）
            cv2.polylines(frame_canvas, [segment.astype(np.int32)], isClosed=True, color=color, thickness=2)
 
            # 绘制边界框和类别信息
            cv2.rectangle(frame_canvas, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), color, 2)
            cv2.putText(frame_canvas, f"{self.classes[cls_]}: {conf:.2f}", (int(box[0]), int(box[1] - 10)),
    ...
    def run_from_cli(file_path=None):
        parser = argparse.ArgumentParser(description="Process an image using VideoPolypDetection.")
        ...
        polyp_detection = VideoPolypDetection()
...
def process_video(video_path, output_path, polyp_detection):
    """
    ...
    Args:
        ...
        polyp_detection (VideoPolypDetection): Instance of the VideoPolypDetection class.
    """
    cap = cv2.VideoCapture(video_path)
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    fps = cap.get(cv2.CAP_PROP_FPS)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 选择输出视频格式
    ...
    while cap.isOpened():
        ...
        processed_frame, _, _ = polyp_detection.process_frame(frame)
...
def video_seg(video_path, output_path):
    video_path = video_path  # 输入视频文件路径
    output_path = output_path  # 输出视频文件路径
    # print("视频路径",video_path)
    # print("结果路径",output_path)
    polyp_detection = VideoPolypDetection()  # 创建PolypDetection实例
    process_video(video_path, output_path, polyp_detection)  # 处理视频并保存结果
    return output_path